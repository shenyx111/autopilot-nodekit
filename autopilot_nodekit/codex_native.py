...
# Final override kept in sync with repo-local skill.
...
# Smart-start/background overrides.
# CODEX_CONFIG_TOML omits job_max_runtime_seconds so NodeKit does not impose a wall-clock worker timeout by default.
...
- Execution loop: `worker-loop` or `launch-background`; use `codex-prepare` / `codex-finish` only for manual debugging or pilot inspection
...
- Repair: operator handles routine `add-repair-task`, `resolve-by-repair`, and `recover-stale`; stop for repeated failures or safety gates
