...
如果你把这个包交给 Codex、Claude Code、Cursor 或其他 AI agent，可以直接给它下面这段。你只需要改 `<在这里写你的任务>` 这一块，其他部分保留。指令要按字面执行，不要让 agent 自己绕过 NodeKit 的任务图、gate 或 verifier：
...
3. 进入我要自动化的项目 workspace。根据“我的任务”创建或检查 PROJECT_PROMPT.md，里面必须写清目标、输入数据、输出要求、最终输出位置、限制条件，以及哪些步骤需要先问用户。
...
9. 完成任务后，必须告诉用户最终结果放在哪里。优先给出明确路径，例如输出目录、报告文件、图表文件、数据文件或模型文件。

10. 如果窗口关闭、token 用完、worker 中断或任务卡住，不要从头重做。先运行 status、next-command 或 recover-stale，按 NodeKit 的恢复逻辑继续。
```
...
下面两条命令由 agent 执行，普通用户通常无需手动运行。这里保留它们，是为了让 agent 知道该用哪些命令启动后台模式：
...
If you give this package to Codex, Claude Code, Cursor, or another AI agent, paste the instruction below. You only need to replace `<write your task here>` and keep the rest. It should be followed literally; the agent should not bypass NodeKit's task graph, gates, verifiers, or recovery flow:
...
3. Go to the project workspace that should be automated. Create or inspect PROJECT_PROMPT.md from "My task". It must state the goal, input data, output requirements, final output location, constraints, and any steps that should ask the user first.
...
9. When the task is complete, tell the user where the final results are. Prefer exact paths to the output directory, report, figures, data files, or model files.

10. If the window closes, tokens run out, a worker stops, or a task gets stuck, do not restart from scratch. Run status, next-command, or recover-stale first, then continue through NodeKit's recovery flow.
```
...
The two commands below are for the agent to run. Most users do not need to run them manually. They are shown so the agent knows how to start background mode:
